defmodule FakeCiWeb.CoreComponents do
  @moduledoc """
  ...
  use Gettext, backend: FakeCiWeb.Gettext
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(FakeCiWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(FakeCiWeb.Gettext, "errors", msg, opts)
    end
  end
  ...
  @doc """
  Renders a status badge for CI builds.
  ...
  def status_badge(assigns) do
    ~H"""
    <span class={[
      "inline-flex items-center px-3 py-1 rounded-full text-sm font-medium gap-1.5",
  ...
  @doc """
  Renders a repository card for the CI dashboard.

  ## Examples

      <.repo_card repository={%{name: "my-app", status: :running}} />
  """
      ...
      <div class="px-4 py-5 sm:px-6">
        <div class="flex items-center justify-between">
          <div class="flex items-center space-x-3">
            <.icon name="hero-code-bracket" class="h-8 w-8 text-gray-400" />
            <div>
          ...
          <div class="flex items-center space-x-4">
            <%= if @repository.started_at do %>
              <span class="flex items-center">
                ...
                Started {format_relative_time(@repository.started_at)}
              </span>
            <% end %>
            <%= if @repository.finished_at do %>
              <span class="flex items-center">
                <.icon name="hero-check-circle" class="h-3 w-3 mr-1" />
                Finished {format_relative_time(@repository.finished_at)}
              </span>
            <% end %>
          </div>
  ...
  @doc """
  Renders a status icon based on the build status.
  """
  attr :status, :atom, required: true
  attr :class, :string, default: "h-3 w-3 mr-1.5"

  def status_icon(assigns) do
    ~H"""
    <%= case @status do %>
      <% :running -> %>
        <div class={["w-4 h-4 animate-spin flex items-center justify-center", @class]}>
          <.icon name="hero-arrow-path" class="w-4 h-4" />
        </div>
      <% :success -> %>
        <.icon name="hero-check-circle" class="w-4 h-4 text-green-400" />
      <% :failure -> %>
        <.icon name="hero-x-circle" class="w-4 h-4 text-red-400" />
      <% :pending -> %>
        <.icon name="hero-clock" class="w-4 h-4 text-yellow-400" />
      <% :cancelled -> %>
        <.icon name="hero-stop-circle" class="w-4 h-4 text-gray-400" />
      <% _ -> %>
  ...
  defp status_text(:success), do: "Passed"
  ...
  @doc """
  Renders a flexible card with status indicator and multiple content slots.
  ...
  """
  attr :id, :string, required: true, doc: "the id attribute for the card"
  attr :status, :atom, required: true, doc: "the status of the card for styling"
  attr :class, :string, default: "", doc: "additional CSS classes for the card"

  slot :header, required: true, doc: "the header content of the card"
  slot :content, required: true, doc: "the main content of the card"
  slot :actions, required: true, doc: "the actions section of the card"
  ...
  def card(assigns) do
    ...
    <div
      class={[
        "bg-white rounded-xl shadow-sm border border-slate-200 hover:shadow-md hover:border-slate-300 transition-all duration-300 overflow-hidden flex flex-col w-full",
        @class
      ]}
      id={@id}
    >
      <div class={["h-1.5 w-full", status_bar_class(@status)]}></div>
      ...
      <div class="p-6 flex-1 flex flex-col">
        ...
        <div :if={@actions != []} class="pt-3 border-t border-slate-100 mt-auto">
          {render_slot(@actions)}
        </div>
      </div>
    </div>
  ...
  def copy_button(assigns) do
    ...
    <button
      type="button"
      class={[
        "text-xs text-slate-500 bg-white px-2 py-1 rounded border font-mono hover:bg-slate-50 hover:text-slate-600 transition-colors cursor-pointer",
        @class
      ]}
  ...
  # Helper function for card status bar colors
  defp status_bar_class(:running), do: "bg-blue-500"
  defp status_bar_class(:success), do: "bg-green-500"
  defp status_bar_class(:failure), do: "bg-red-500"
  defp status_bar_class(:pending), do: "bg-yellow-500"
  defp status_bar_class(:cancelled), do: "bg-gray-500"
  defp status_bar_class(_), do: "bg-gray-300"
